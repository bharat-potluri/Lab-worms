...
const SignupForm = () => {
  ...
  // Add this function to render validation errors
  const renderValidationErrors = (fieldErrors) => {
...
<MDBInput
      ...
      onChange={handleInputChange}
      required
      className={`mb-2 ${validationErrors.password.length > 0 ? 'is-invalid' : ''}`}
    />
    ...
    <MDBRow className="align-items-end">
      ...
      <MDBCol>
        <MDBInput
          ...
          onChange={handleInputChange}
          required
          className={`mb-2 ${validationErrors.phoneNumber.length > 0 ? 'is-invalid' : ''}`}
        />
        {renderValidationErrors(validationErrors.phoneNumber)}
      </MDBCol>
    </MDBRow>
