  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // If not found, check the auth mapping
        if (!userDoc.exists()) {
          console.log('User document not found directly, checking auth mapping...');
          const mappingDoc = await getDoc(doc(db, 'authMappings', currentUser.uid));
          
          if (mappingDoc.exists()) {
            const mappingData = mappingDoc.data();
            const alternateUid = mappingData.emailAuthUid || mappingData.phoneAuthUid;
            
            if (alternateUid) {
              // console.log('Found alternate UID:', alternateUid);
              userDoc = await getDoc(doc(db, 'users', alternateUid));
            }
          }
        }

        if (!userDoc.exists()) {
          throw new Error('User profile not found after checking authmappings');
        }
