...
const QuestionsPage = () => {
  ...
  const onQuestionClick = (question) => {
    handleQuestionClick(question, navigate);
  };

  // Modified handleQuestionClick function
const handleQuestionClick = async (question, navigate) => {
  try {
      ...
      if (!userDocSnap.exists()) {
          const mappingDoc = await getDoc(doc(db, 'authMappings', user.uid));
          ...
          if (mappingDoc.exists()) {
              ...
              if (alternateUid) {
                  userDocSnap = await getDoc(doc(db, 'users', alternateUid));
              }
          }
      }
        ...
        <MDBCardBody>
          <MDBRow>
            ...
            <h5 
                ...
                onClick={() => onQuestionClick(question)}  // Updated here
