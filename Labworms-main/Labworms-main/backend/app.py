...
CORS(app, resources={
    r"/*": {
        "origins": [
            "https://leetcodefrontend.uc.r.appspot.com",
            "https://labworms.fun",
            "https://www.labworms.fun"
        ],
...
def home():
    return 'Leetcode5560-cors-updated-4'
...
# In app.py, modify the register endpoint
...
def register():
    try:
        ...
        user_data = {
            ...
            'phoneAuthUid': data.get('phoneAuthUid')  # Store the phone auth UID
        }
        ...
        try:
            ...
            # If phone auth UID is provided, create a mapping
            if data.get('phoneAuthUid'):
                mapping_data = {
                    'emailAuthUid': data['uid'],
                    'createdAt': datetime.datetime.now().isoformat()
                }
                db.collection('authMappings').document(data['phoneAuthUid']).set(mapping_data)
                
                # Create reverse mapping
                reverse_mapping = {
                    'phoneAuthUid': data['phoneAuthUid'],
                    'createdAt': datetime.datetime.now().isoformat()
                }
                db.collection('authMappings').document(data['uid']).set(reverse_mapping)
...
@app.route('/api/update-subscription', methods=['POST', 'OPTIONS'])
def update_subscription():
    ...
    try:
        ...
        token = auth_header.split(' ')[1]
        print(f"Got token for subscription update: {token[:10]}...")
        
        decoded_token = admin_auth.verify_id_token(token)
        uid = decoded_token['uid']
        print(f"Decoded UID for subscription update: {uid}")
        
        # First try direct user document
        user_doc_ref = db.collection('users').document(uid)
        user_doc = user_doc_ref.get()
        
        # If not found, check auth mappings
        if not user_doc.exists:
            print(f"User not found directly, checking auth mappings...")
            mapping_doc = db.collection('authMappings').document(uid).get()
            
            if mapping_doc.exists:
                mapping_data = mapping_doc.to_dict()
                alternate_uid = mapping_data.get('emailAuthUid') or mapping_data.get('phoneAuthUid')
                
                if alternate_uid:
                    print(f"Found alternate UID: {alternate_uid}")
                    user_doc_ref = db.collection('users').document(alternate_uid)
                    user_doc = user_doc_ref.get()
        
        if not user_doc.exists:
            print(f"No user document found, creating one...")
            try:
                ...
                user_data = {
                    ...
                    'firstName': auth_user.display_name.split()[0] if auth_user.display_name else '',
                    'lastName': ' '.join(auth_user.display_name.split()[1:]) if auth_user.display_name else '',
                }
                
                # Save to Firestore
                user_doc_ref = db.collection('users').document(uid)
                user_doc_ref.set(user_data)
                print(f"Created new user document for {auth_user.email}")
                
            except Exception as e:
                print(f"Error creating user document: {str(e)}")
                return jsonify({'success': False, 'error': 'Could not create user document'}), 500
        ...
        try:
            ...
            print(f"Updated subscription status for user document")
        ...
        try:
            ...
            if not user_doc.exists:
                ...
                if mapping_doc.exists:
                    ...
                    if alternate_uid:
                        print(f"Found alternate UID: {alternate_uid}")
                        user_doc = db.collection('users').document(alternate_uid).get()

            # If still not found, try to get user info from Firebase Auth and create document
            if not user_doc.exists:
                print(f"No user document found, creating one...")
                try:
                    auth_user = admin_auth.get_user(uid)
            ...
            if not user_doc.exists:
                print(f"Still no user document found after all attempts")
                return jsonify({'success': False, 'error': 'User not found'}), 404
                
            user_data = user_doc.to_dict()
            print(f"Found/Created user data: {user_data.get('email')}")
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='subscription',
                success_url=f"{BaseURL}/subscription/success",
                cancel_url=f"{BaseURL}/proFailed",
